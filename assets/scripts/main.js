...
function initializeServiceWorker() {
  ...
  window.addEventListener("load", async (event) => {
    try {
      ...
      console.error("The service worker registration has failed.");
    }
  });
...
async function getRecipes() {
  ...
  return new Promise(async (resolve_function, reject_function) => {
    for (const index in RECIPE_URLS) {
      const recipe_url = RECIPE_URLS[index];
      try {
        const response = await fetch(recipe_url);
        const json_data = await response.json();
        recipe_array.push(json_data);
        if (index == (RECIPE_URLS.length - 1)) {
          saveRecipesToStorage(recipe_array);
          resolve_function(recipe_array);
        }
      } catch (error) {
        console.log(error);
        reject_function(error);
      }
    }
  });
  /**************************/
